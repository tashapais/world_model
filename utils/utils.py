...
def _load_checkpoint_data(checkpoint_path):
    """Load model state dict and state config from either a directory or a single .pth file."""
    import torch
    ...
    if p.is_dir():
        ...
        model_sd = ckpt['model']
        state_cfg = {'config': ckpt.get('config', {}), 'step': ckpt.get('step')}
    return model_sd, state_cfg
...
def load_videotokenizer_from_checkpoint(checkpoint_path, device, model = None, is_distributed = False):
    ...
    model_sd, state_cfg = _load_checkpoint_data(checkpoint_path)
