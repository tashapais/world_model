...
class LatentActionsEncoder(nn.Module):
    def __init__(self, frame_size=(128, 128), patch_size=8, embed_dim=128, num_heads=8,
                 hidden_dim=256, num_blocks=4, action_dim=3, use_windowed_attention=False, use_rope=False, use_adaln_zero=False):
        super().__init__()
        H, W = frame_size
...
class LatentActionsDecoder(nn.Module):
    def __init__(self, frame_size=(128, 128), patch_size=8, embed_dim=128, num_heads=8,
                 hidden_dim=256, num_blocks=4, conditioning_dim=3, use_rope=False, use_adaln_zero=False):
        super().__init__()
        H, W = frame_size
        grid_size = (H // patch_size, W // patch_size)
        self.patch_embed = PatchEmbedding(frame_size, patch_size, embed_dim)
...
class LatentActionModel(nn.Module):
    def __init__(self, frame_size=(128, 128), n_actions=8, patch_size=8, embed_dim=128,
                 num_heads=8, hidden_dim=256, num_blocks=4, use_windowed_attention=False, use_rope=False, use_adaln_zero=False):
        super().__init__()
        ...
        self.encoder = LatentActionsEncoder(frame_size, patch_size, embed_dim, num_heads, hidden_dim, num_blocks, action_dim=self.action_dim, use_windowed_attention=use_windowed_attention, use_rope=use_rope, use_adaln_zero=use_adaln_zero)
        self.quantizer = FiniteScalarQuantizer(latent_dim=self.action_dim, num_bins=NUM_LATENT_ACTIONS_BINS)
        self.decoder = LatentActionsDecoder(frame_size, patch_size, embed_dim, num_heads, hidden_dim, num_blocks, conditioning_dim=self.action_dim, use_rope=use_rope, use_adaln_zero=use_adaln_zero)
